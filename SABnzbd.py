#!/usr/bin/python -OO
# FIXME: conflictin declares across project, should all python files use: '#!/usr/bin/env python -OO' ?
#
...
SQLITE_DLL = True
try:
    from sqlite3 import version as sqlite3_version  # @UnusedImport
except:
    try:
        from pysqlite2.dbapi2 import version as sqlite3_version  # @UnresolvedImport @UnusedImport @Reimport
    except:
        if os.name != 'nt':
...
import sabnzbd  # @UnusedImport
import sabnzbd.lang  # @UnusedImport
import sabnzbd.interface
from sabnzbd.constants import *  # @UnusedWildImport
...
def FORCELOG(txt):
    # TODO: looks to be an unused function?
    global _first_log
    if _first_log:
...
class FilterCP3:
    ...
    def filter(self, record):
        ...
        # TODO: Drop hack since python 2.4 isn't supported
        # Python2.4 fix
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    sabnzbd.DIR_APPDATA = sabnzbd.DIR_APPDATA.encode(codepage)  # @UndefinedVariable
                    sabnzbd.DIR_HOME = sabnzbd.DIR_HOME.encode(codepage)  # @UndefinedVariable
                    ok = True
                except:
...
def check_resolve(host):
    ...
    try:
        _info = socket.getaddrinfo(host, None)
    except:
        # Does not resolve
        return False
    return True
...
def commandline_handler(frozen=True):
    ...
    service = ''
    sab_opts = []
    serv_inst = False  # @UnusedVariable?
...
def print_ip_addresses():
    # TODO: should this be changed since some people would not want to be hitting up google for any reason (also could be blocked.. china) -- also does not look like its actually used anywhere
    try:
...
def main():
                ...
                if new_instance or not check_for_sabnzbd(url, upload_nzbs, autobrowser):
                    ...
                    if newport > 0:
                        ...
                        if https_port:
                            ...
                            # FIXME: This is flagged as not being used
                            http_port = newport
        except:
            Bail_Out(browserhost, cherryport, '49')
    ...
    try:
        rollover_log = log_handler(sabnzbd.LOGFILE,
                                   'a+',
                                   logsize,
    ...
    # TODO: why isn't print_ip_addresses() used here?
    try:
        s_ipv4 = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        s_ipv4.connect(("google.com", 80))
        logging.info('IP address = %s', s_ipv4.getsockname()[0])
    ...
    while not sabnzbd.SABSTOP:
        ...
        if cherrypy.engine.execv:
            ...
            if sabnzbd.DARWIN:
                ...
                # FIXME: when executing from sources on osx, after a restart, process is detached from console
...
if __name__ == '__main__':
    ...
    if sabnzbd.WIN32:
        ...
        try:
            ...
            from sabnzbd.osxmenu import SABnzbdDelegate  # @UnusedImport
