...
class TestScriptDirNotAppDirValidator(unittest.TestCase):
    ...
    @patch(
        "sabnzbd.validators.script_dir_not_appdir_validator.sabnzbd.misc.helpful_warning"
    )
    def test_script_dir_not_appdir_validator_app_directory_warning(self, mock_warning):
        ...
        with (
            ...
            error, result = self.validator.validate(
                self.root, "/app/scripts", self.default
            )
    ...
    def test_script_dir_not_appdir_validator_non_app_directory_no_warning(
        self, mock_warning
    ):
        ...
        with (
            ...
            error, result = self.validator.validate(
                self.root, "/custom/scripts", self.default
            )
            self.assertIsNone(error)
    ...
    def test_script_dir_not_appdir_validator_instance(self):
        ...
        with (
            patch.object(
                script_dir_not_appdir_validator, "_same_directory", return_value=False
            ),
            patch(
                "sabnzbd.validators.script_dir_not_appdir_validator.sabnzbd.misc.helpful_warning"
            ) as mock_warning,
            ...
            error, result = script_dir_not_appdir_validator(
                self.root, "/custom/scripts", self.default
            )
            self.assertIsNone(error)
            self.assertEqual(result, "/custom/scripts")
        ...
        with (
            patch.object(
                script_dir_not_appdir_validator, "_same_directory", return_value=True
            ),
            patch(
                "sabnzbd.validators.script_dir_not_appdir_validator.sabnzbd.misc.helpful_warning"
            ) as mock_warning,
    ...
    def test_script_dir_not_appdir_validator_various_paths(self, mock_warning):
        ...
        for path, is_app_directory, should_warn in test_cases:
            with self.subTest(path=path):
                with (
                    patch.object(
                        self.validator, "_same_directory", return_value=is_app_directory
                    ),
                    patch(
                        "sabnzbd.validators.script_dir_not_appdir_validator.sabnzbd.DIR_PROG",
                        "/app",
