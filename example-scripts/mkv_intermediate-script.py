...
def mkv_has_subs(mkv_file: str) -> bool:
    """Check if an MKV file has subtitle tracks.
    Ugly hack ahead: Just count the nubmer of occurences of "S_TEXT/UTF8" in the first 10_000 bytes of the file.
    """
    ...
    try:
        with open(mkv_file, "rb") as f:
            data = f.read(10_000)
            count_subs = data.count(b"S_TEXT/UTF8") + data.count(b"S_TEXT/ASS")
            if count_subs > 0:
                return True
            else:
                return False
    except Exception as e:
        print(f"Error reading MKV file {mkv_file}: {e}")
        return False
...
# find extension of biggest file

_, ext = os.path.splitext(biggest_file)
ext = ext.lower()
if ext == ".mkv":
    if mkv_has_subs(biggest_file):
        prio = 2  # higher priority
    else:
        prio = -1  # lower priority
