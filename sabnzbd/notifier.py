...
def register_growl(growl_server, growl_password):
    ...
    growler = GrowlNotifier(
        ...
        notifications=[T("%s") % NOTIFICATION[key] for key in NOTIFY_KEYS],
...
def send_growl(title, msg, gtype, test=None):
    ...
    for n in (0, 1):
        ...
        if _GROWL:
            ...
            try:
                ret = _GROWL.notify(noteType=T("%s") % NOTIFICATION.get(gtype, "other"), title=title, description=msg)
                if ret is None or isinstance(ret, bool):
                    return None
                elif ret[0] == "401":
                    _GROWL = False
...
if _HAVE_CLASSIC_GROWL:
    ...
    def send_local_growl(title, msg, gtype):
        ...
        global _local_growl
        if not _local_growl:
            notes = [T("%s") % NOTIFICATION[key] for key in NOTIFY_KEYS]
            _local_growl = Growl.GrowlNotifier(
                applicationName="SABnzbd", applicationIcon=_OSX_ICON, notifications=notes, defaultNotifications=notes
            )
            _local_growl.register()
        _local_growl.notify(T("%s") % NOTIFICATION.get(gtype, "other"), title, msg)
        return None
...
def send_notification_center(title, msg, gtype):
    ...
    if tool:
        try:
            command = [tool, "-title", title, "-message", msg, "-group", T("%s") % NOTIFICATION.get(gtype, "other")]
...
def send_prowl(title, msg, gtype, force=False, test=None):
    ...
    title = T("%s") % NOTIFICATION.get(gtype, "other")
...
def send_pushover(title, msg, gtype, force=False, test=None):
    ...
    if -3 < prio < 2:
        body = {"token": apikey, "user": userkey, "device": device, "title": title, "message": msg, "priority": prio}
        return do_send_pushover(body)
...
def send_pushbullet(title, msg, gtype, force=False, test=None):
    ...
    title = "SABnzbd: {}".format(T("%s") % NOTIFICATION.get(gtype, "other"))
