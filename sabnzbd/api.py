...
from sabnzbd.misc import loadavg, to_units, int_conv, time_format, cat_convert, create_https_certificates, calc_age
from sabnzbd.filesystem import diskspace, get_ext, get_filename, globber, globber_full, clip_path, remove_all
...
def _api_translate(name, output, kwargs):
    ...
    return report(output, keyword="value", data=T("%s") % kwargs.get("value", ""))
...
def Ttemplate(txt):
    ...
    if txt in _SKIN_CACHE:
        ...
        tra = T("%s") % SKIN_TEXT.get(txt, txt).replace('"', "&quot;").replace("'", "&apos;")
        _SKIN_CACHE[txt] = tra
        return tra
...
def get_active_history(queue=None, items=None):
    ...
    for nzo in queue:
        ...
        item = {}
        item["completed"], item["name"], item["nzb_name"], item["category"], item["pp"], item["script"], item[
            "report"
        ], item["url"], item["status"], item["nzo_id"], item["storage"], item["path"], item["script_log"], item[
            "script_line"
        ], item[
            "download_time"
        ], item[
